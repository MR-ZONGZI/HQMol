...
SCHEDULER_NAME = "scheduler_state.pt"
...
class ModelArguments:
    ...
    himol_fuse: str = field(default=None)
    num_head: int = field(default=None)
    ...
    pretrain_mm_node_mlp_adapter: Optional[str] = field(default=None)
    pretrain_mm_motif_mlp_adapter: Optional[str] = field(default=None)
    pretrain_mm_graph_mlp_adapter: Optional[str] = field(default=None)
    mm_projector_type: Optional[str] = field(default='linear')
    mm_node_projector_type: Optional[str] = field(default='linear')
    mm_motif_projector_type: Optional[str] = field(default='linear')
    mm_graph_projector_type: Optional[str] = field(default='linear')
...
def get_keys_to_match(graph_tower, himol_fuse):
    if graph_tower == "moleculestm":
        ...
        raise ValueError(f"Unsupported GRAPH_TOWER value: {graph_tower}. Expected values are 'moleculestm' or 'himol'.")
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer, output_dir: str):
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        ...
        from ..himol_loader_utils import get_graph_tower
        from ..himol_loader_utils import get_himol_fuse

        graph_tower = get_graph_tower()
        himol_fuse = get_himol_fuse()

        keys_to_match = get_keys_to_match(graph_tower, himol_fuse)
        ...
        for key in keys_to_match:
            weight_to_save[key] = get_mm_adapter_state_maybe_zero_3(trainer.model.named_parameters(), [key])

        graph_weights = {}
        if getattr(trainer.args, 'tune_graph_tower', False):
            print("Trying to find the parameters of graph tower...")
            graph_weights = get_mm_adapter_state_maybe_zero_3(trainer.model.named_parameters(), ['graph_tower'])
        else:
        ...
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            if current_folder.startswith('checkpoint-'):
                ...
                for key, weight in weight_to_save.items():
                    torch.save(weight, os.path.join(mm_projector_folder, f'{current_folder}_{key}.bin'))
            else:
                print("Saving the last Adapters...")
                for key, weight in weight_to_save.items():
                    torch.save(weight, os.path.join(output_dir, f'{key}.bin'))

            if graph_weights:  # save when weights are not none
                print("Saving the last Graph Tower...")
...
def train():
    ...
    if model_args.graph_tower is not None:
        ...
        if model_args.graph_tower == "moleculestm":
            projectors = [model.get_model().mm_projector]
        elif model_args.graph_tower == "himol" and model_args.himol_fuse is not None:
            himol_fuse = getattr(model_args, "himol_fuse")
            if himol_fuse == "0":
                projectors = [
                    model.get_model().mm_node_projector,
                    model.get_model().mm_motif_projector,
                    model.get_model().mm_graph_projector
                ]
            elif himol_fuse == "1":
                projectors = [
                    model.get_model().mm_motif_projector,
            ...
            elif himol_fuse == "fuse":
                projectors = [model.get_model().mm_projector]
            else:
                raise ValueError(f"Unsupported HIMOL_FUSE value: {himol_fuse}.")
        else:
            raise ValueError(f"Unsupported GRAPH_TOWER value: {model_args.graph_tower}")
        ...
        if model_args.tune_graph_tower:
            ...
            for name, param in graph_tower.named_parameters():
                if "hierachy" not in name and "norm" in name or "attention" in name:
                    print(f"Graph Tower {name} requires grad")
        ...
        def on_save(self, args, state, control, **kwargs):
            if os.environ.get("RANK", "0") == "0":
                ...
                if scheduler is None:
                    print("[SchedulerStateCallback] No lr_scheduler found in kwargs during on_save.")
                    return
                ...
                print(f"[SchedulerStateCallback] Saved scheduler state to {save_path}")

        def on_train_begin(self, args, state, control, **kwargs):
            if args.resume_from_checkpoint is not None and isinstance(args.resume_from_checkpoint, str):
                print("[SchedulerStateCallback] resume_from_checkpoint is set to: {args.resume_from_checkpoint}")
                load_path = os.path.join(args.resume_from_checkpoint, SCHEDULER_NAME)
                print(f"[SchedulerStateCallback] Computed scheduler state path: {load_path}")
                if os.path.exists(load_path):
                    scheduler = kwargs.get("lr_scheduler")
                    print(f"[SchedulerStateCallback] Scheduler state file found. Proceeding to load...")
                    if scheduler is None:
                        print("[SchedulerStateCallback] lr_scheduler not found in kwargs during on_train_begin.")
                        return
                    scheduler_state = torch.load(load_path)
                    scheduler.load_state_dict(scheduler_state)
                    print(f"[SchedulerStateCallback] Successfully loaded scheduler state from {load_path}")

    # summary model parameter size
    if training_args.local_rank in [-1, 0]:
        from transformers import logging
        logging.set_verbosity_info()
        print(f"Number of paramrters: {model.num_parameters()}")
    
    trainer = LLaVATrainer(model=model,
                    tokenizer=tokenizer,
                    args=training_args,
                    callbacks=[SaveCallback(), SchedulerStateCallback()],
                    **data_module)
