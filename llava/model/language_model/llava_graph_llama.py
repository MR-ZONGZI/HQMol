...
from llava.himol_loader_utils import LlavaMetaModel, LlavaMetaForCausalLM
...
class LlavaLlamaModel(LlavaMetaModel, LlamaModel):
    # `config_class` 是一个类属性，指定了该模型使用的配置类。
    # `LlavaGraphLlamaConfig` 是这个模型的配置类。
    config_class = LlavaGraphLlamaConfig

    def __init__(self, config: LlamaConfig):
        # 调用父类的构造函数，初始化 `LlavaMetaModel` 和 `LlamaModel` 类。
        # `super(LlavaLlamaModel, self).__init__(config)` 会依次调用 `LlavaMetaModel` 和 `LlamaModel` 的构造函数。
        # 其中，`config` 是传递给父类构造函数的配置对象，通常包含模型参数、超参数等信息。
...
class LlavaGraphLlamaForCausalLM(LlamaForCausalLM, LlavaMetaForCausalLM):
    ...
    def forward(
        ...
        # 准备输入数据，特别是多模态输入（如图形数据）。
        # `prepare_inputs_labels_for_multimodal` 用于将图形数据整合到输入中。
        ...
        # 通过 `LlavaLlamaModel` 进行推理，得到模型输出。
