...
from .moleculeSTM_gnn_model import GNN_graphpred as STM_GNN_graphpred, GNN as STM_GNN
from .himol_gnn_model import GNN_graphpred as himol_GNN_graphpred, GNN as himol_GNN
...
def build_graph_tower(graph_tower_cfg, **kwargs):
    ...
    if graph_tower.startswith("graphmvp"):
    ...
    elif graph_tower.startswith("himol"):
        molecule_hierachy_model = himol_GNN(
            num_layer=graph_tower_cfg.gin_num_layers,
            emb_dim=graph_tower_cfg.gin_hidden_dim,
            JK='last', # default to 'last'
            drop_ratio=graph_tower_cfg.drop_ratio,
        ...
        return himol_GNN_graphpred(
            emb_dim=graph_tower_cfg.gin_hidden_dim,
            graph_pooling=graph_tower_cfg.graph_pooling,
            molecule_hierachy_model=molecule_hierachy_model,
            init_checkpoint=graph_tower_cfg.init_checkpoint,
            himol_fuse=graph_tower_cfg.himol_fuse,
            num_head=graph_tower_cfg.num_head,
        )
