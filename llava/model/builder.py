    ...
    **kwargs
    ...
    if 'llava' in model_name.lower():
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            ...
            if mm_encoder_cfg and "init_checkpoint" in mm_encoder_cfg:
                try:
                    checkpoint = torch.load(mm_encoder_cfg["init_checkpoint"], map_location="cpu")
                    ...
                    adapted_state_dict = {
                        key if key.startswith("model.graph_tower") else f"model.graph_tower.{key}": value 
                        for key, value in state_dict.items()
                    }
                    adapted_state_dict = {k: v.to("cpu") for k, v in adapted_state_dict.items()}
                    model.load_state_dict(adapted_state_dict, strict=False)

                except Exception as e:
                    raise RuntimeError(f"Failed to load graph encoder: {str(e)}")
            else:
                print("Skipping loading graph encoder parameters as no init_checkpoint is provided in mm_encoder_cfg.")
            ...
            for name, param in model.named_parameters():
                if torch.isnan(param).any() or torch.isinf(param).any():
                    print(f"Parameter {name} contains invalid values!")
