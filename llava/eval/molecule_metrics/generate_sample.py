...
from llava.datasets.smiles2higraph import smiles2higraph
...
def _convert_dict_to_Data(data_dict: Dict) -> Data:
    return Data(
        ...
        num_nodes_array=data_dict['num_nodes']
    )
    ...
    with open(args.in_file, "rb") as f:
        ...
        for i, raw in enumerate(list_data_dict):
            if args.task == "retrosynthesis":
                graph = get_graph(selfies2smiles(raw['input']))
                if args.add_selfies:
                ...
                graph = get_graph(selfies2smiles(reactant))
                ...
                graph = get_graph(selfies2smiles(input_selfies))
                batch.append((instruction, graph, raw['output']))
            else:
...
def main(args):
    ...
    mm_encoder_cfg = MM_ENCODER_CFG(init_checkpoint=args.graph_checkpoint_path, gin_hidden_dim=args.gin_hidden_dim, himol_fuse=args.himol_fuse, num_head=args.num_head)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--graph_tower", type=str, default=None)
    parser.add_argument("--himol_fuse", type=str, default=None)
